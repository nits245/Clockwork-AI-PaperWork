...
export default function EditDoc(item) {
  ...
  const fetchDataByType = async () => {
    try {
      axios
        .get(`${process.env.REACT_APP_BACKEND_URL}/customise-document/${id}`)
        .then((res) => {
  ...
  const updateTemplateEndpoint = (id) => {
    try {
      axios
        .put(`${process.env.REACT_APP_BACKEND_URL}/customise-document/${id}`, {
          type: data.type,
          title: content,
          content: terms,
  ...
  const updateSignatureConfigEndpoint = (id, savedItem) => {
    try {
      ...
      console.error("Error updating signature config:", error);
    }
  };
  ...
  const updatePartiesToTheEndpoint = (id, partyList) => {
    try {
      ...
      console.error("Error sending parties:", error);
    }
  };
